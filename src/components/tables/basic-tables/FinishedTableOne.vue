          ...
          <tr v-for="(settlement, index) in paginatedItems" :key="index"
            ...
            <td class="px-5 py-4 sm:px-6 relative">
              <div class="absolute top-0 left-0 inline-flex gap-2">
                <span v-if="settlement.tipo != LiquidacionTypes.NEGOCIO_LIBERADO && settlement.factura_ciaros != 0"
                  class="inline-block px-2 py-0.5 text-xs font-semibold rounded-br-lg"
                  :class="{ 'text-yellow-800 bg-yellow-200': invoiceOrNote(settlement) == 'No aplica', 'text-error-800 bg-error-200': invoiceOrNote(settlement) == 'No pagada', 'text-emerald-800 bg-emerald-200': invoiceOrNote(settlement) == 'Pagada' }">
                  {{ invoiceOrNote(settlement) }}
                </span>
                <span
                  v-if="settlement.tipo == LiquidacionTypes.CONSOLIDADO && settlement.factura_ciaros == 0 && settlement.FinalizadaNumeroLiquidacion"
                  class="inline-block px-2 py-0.5 text-xs font-semibold rounded-br-lg bg-indigo-800 text-indigo-200">
                  Pagado sin factura
                </span>
                <span
                  v-if="settlement.tipo == LiquidacionTypes.ARCHIVADO && settlement.factura_ciaros == 0 && settlement.FinalizadaNumeroLiquidacion"
                  class="inline-block px-2 py-0.5 text-xs font-semibold rounded-br-lg bg-indigo-800 text-indigo-200">
                  Pagado sin factura
                </span>
                <span
                  v-if="settlement.tipo == LiquidacionTypes.NEGOCIO_LIBERADO && !settlement.FinalizadaNumeroLiquidacion"
                  class="inline-block px-2 py-0.5 text-xs font-semibold rounded-br-lg bg-amber-300 text-amber-800">
                  Por facturar
                </span>
                <span v-if="settlement.ori && settlement.ori.trim() == 'AP'"
                  class="text-rose-800 bg-rose-200 px-2 py-0.5 text-xs font-semibold rounded-lg">
                  Vida
                </span>
              </div>
              ...
              <div class="flex items-center gap-3">
                <div>
                  <span class="block font-medium text-gray-800 text-theme-xs dark:text-white/90">
                    {{ settlement.FinalizadaNumeroLiquidacion || "No fue liquidada" }}
                  </span>
                </div>
              </div>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <div class="flex items-center gap-3">
                <div>
                  <span class="block font-medium text-gray-800 text-theme-xs dark:text-white/90">
                    {{ settlement.Finalizada && settlement.Finalizada.fecha_pago ?
                      dateFormatter(settlement.Finalizada.fecha_pago) : "N/A" }}
                  </span>
                </div>
              </div>
            </td>
...
import { KanbanStates, LiquidacionStates, LiquidacionTypes, type LiquidacionMappedTS, type LiquidacionRowTS, type LiquidacionTS } from '@/interfaces/LiquidacionesTS';
...
const props = defineProps<{ settlements: LiquidacionMappedTS[], liquidator: boolean, pdf_generator: boolean, count: number, type: string }>();
...
const getSettlementList = async (page: number, limit: number) => {
  ...
  if (filter.value === 'Pagado sin factura') filters.state = filter.value;
  if (filter.value === 'Por facturar') filters.state = filter.value;
  if (filter.value === 'Pagado') filters.state = filter.value;
  if (filter.value === 'No pagado') filters.state = filter.value;
  if (filter.value === 'No aplica') filters.state = filter.value;
...
const invoiceOrNote = (settlement: LiquidacionMappedTS) => {
  if (settlement.estado == LiquidacionStates.EMITIDO && settlement.tipo == LiquidacionTypes.CONSOLIDADO && settlement.endoso?.trim() == "RES") {
    return 'No aplica';
  }

  if (!settlement.Finalizada && settlement.estado == LiquidacionStates.EMITIDO && settlement.tipo == LiquidacionTypes.CONSOLIDADO) {
    return 'Pagada';
  }

  if (settlement.Finalizada) {
    if (settlement.estado == LiquidacionStates.ARCHIVADA && settlement.tipo == LiquidacionTypes.ARCHIVADO && settlement.Finalizada.kanban == KanbanStates.ARCHIVADA) {
      return 'Pagada';
    }
    if (settlement.Finalizada.fecha_pago || settlement.Finalizada.kanban == KanbanStates.PAGADA) {
      return 'Pagada';
    }
    return 'No pagada';
  } else {
