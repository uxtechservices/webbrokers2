    ...
    >
      <div class="dz-message m-0!">
        ...
        <h4 class="mb-3 font-semibold text-gray-800 text-theme-xl dark:text-white/90">
          Arrastrar y soltar archivo XLSX
        </h4>
        <span
          class="mx-auto mb-5 block w-full max-w-[290px] text-sm text-gray-700 dark:text-gray-400"
        >
          Busca tu archivo de tipo Excel o XLSX, arrastra y suelta en este recuadro
        </span>
...
onMounted(() => {
  ...
  dropzoneInstance.value = new Dropzone(`#${dropzoneId}`, {
    url: '/', // No necesitamos una URL porque no subiremos el archivo
    thumbnailWidth: 150,
    maxFilesize: 50,
    acceptedFiles: '.xls,.xlsx',
    autoProcessQueue: false, // No subimos el archivo automáticamente
    init: function () {
      this.on('addedfile', async (file) => {
        excelData.value = file;
      })
    },
  })

  event.on('request-excel', () => {
    event.emit('send-excel', excelData.value);
  })
})
