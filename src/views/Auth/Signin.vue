              ...
              <div>
                <form @submit.prevent="handleSubmit">
                  <div class="space-y-5">
                    ...
                    <!-- Button -->
                    <Alert v-if="errorAlert" variant="error" title="Error al intentar autenticarse"
          ...
          <div class="flex items-center justify-center z-1">
            ...
            <div class="flex flex-col items-center max-w-xs">
              <router-link to="/" class="block mb-4">
                <h1 class="text-white font-bold text-3xl">Sistema Ciaros</h1>
              </router-link>
              <p class="text-center text-gray-400 dark:text-white/60">
                Panel de Administración y Finanzas.
              </p>
            </div>
          </div>
        </div>
...
import { setToken, setUserData } from '@/libs/authentication'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const { data } = await instance.post('/users/auth', { correo: email.value, password: password.value }, {});
