...
const currentPageTitle = ref("Negocios pendientes por facturar");
...
const getSettlementList = async () => {
  ...
  try {
    ...
    const { data } = await instance.get('/settlements/search-by/Negocio pendiente por facturar?page=1&limit=5', {
      headers: {
